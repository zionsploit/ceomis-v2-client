...
import { actionDeleteContractorById } from "./page.form_action"
...
const contractorsDeleteFormSchema = yup.object().shape({
    user_password: yup.string().required("Password is required")
})

export default function ConstractorsDeleteForm({
    session_data,
    id
}: Readonly<{session_data: SessionData, id: number}>) {
    const [state, formAction, pending] = useActionState(actionDeleteContractorById, { message: ResponseDefaultMessage.None, response_data: "" })
    ...
    return <>
        <Formik
            ...
            validationSchema={contractorsDeleteFormSchema}
            onSubmit={(value) => {
                startTransition(() => {
                    formAction({
                         data: {
        ...
        >
            {({values, errors, handleBlur, handleChange, touched, handleSubmit}) => (
                <Form>
                    ...
                    />
                    <Flex gap="md">
                        <Button component={Link} href={'/dashboard/contractors'} variant="outline" size="xs" mt="sm">Cancel</Button>
