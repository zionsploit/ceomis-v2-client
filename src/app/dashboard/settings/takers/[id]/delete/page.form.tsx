...
import { actionDeleteTakersById } from './page.form_action'
...
export default function TakersDeleteForm({
    session_data,
    id
}: Readonly<{ session_data: SessionData, id: number }>) {
    const [state, formAction, pending] = useActionState(actionDeleteTakersById, {message: ResponseDefaultMessage.None, response_data: ""})
    ...
    useEffect(() => {
        ...
        if (stateMessage == ResponseDefaultMessage.Success) {
            ...
            redirect("/dashboard/settings/takers")
        }
