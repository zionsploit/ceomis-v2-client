...
import { actionDeleteBarangayById } from "./page.form_action"
...
const barangayDeleteFormSchema = yup.object().shape({
    user_password: yup.string().required("Password is required")
})

export default function BarangayDeleteForm({
    session_data,
    id
}: Readonly<{session_data: SessionData, id: number}>) {
    const [state, formAction, pending] = useActionState(actionDeleteBarangayById, {
    ...
    useEffect(() => {
        ...
        if (stateMessage == ResponseDefaultMessage.Success) {
            ...
            redirect("/dashboard/settings/barangays")
        }
    ...
    return <>
        <Formik
            ...
            validationSchema={barangayDeleteFormSchema}
            onSubmit={(value) => {
                startTransition(() => {
                    formAction({
                         data: {
        ...
        >
            {({values, errors, handleBlur, handleChange, touched, handleSubmit}) => (
                <Form>
                    ...
                    />
                    <Flex gap="md">
                        <Button component={Link} href={'/dashboard/settings/barangays'} variant="outline" size="xs" mt="sm">Cancel</Button>
