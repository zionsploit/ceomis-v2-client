...
export default function InchargeTable({
    incharge
}: Readonly<{incharge: Array<ResponseIncharge>}>) {
    const pageTitleContext = useContext(PageTitleContext)
    const [inchargeData, setInchargeData] = useState<Array<ResponseIncharge>>(incharge)
    const [searchLoading, setSearchLoading] = useState<boolean>(false)
    const [searchData, setSearchData] = useState<string>("")

    const OnDebounceHandlerSearchData = useDebouncedCallback(async (query: string) => {
        setSearchLoading(true)
        setInchargeData(await getSearchResults(incharge, query, "name"))
    ...
    useEffect(() => {
        pageTitleContext.mutateState({
            title: "Incharge",
            description: "A list of all the incharges registered in the system."
        })
    ...
    return <>
        <Group w={rem("100%")}>
            <TextInput flex={1} 
                disabled={incharge.length == 0}
                placeholder="Search here ..."
                onChange={OnHandlerSearchChange}
                value={searchData}
                rightSection={searchLoading && <Loader size="xs" />}
            />
            <Button component={Link} href="incharge/add" size="sm" leftSection={<IconPlus />}>ADD TYPE</Button>
        </Group>
        <Table mt="xl" highlightOnHover>
            <Table.Thead>
                <Table.Tr>
                    <Table.Th w={px("50")}>#</Table.Th>
                    <Table.Th flex={1}>In-charge</Table.Th>
                </Table.Tr>
            </Table.Thead>
            <Table.Tbody>
                {inchargeData.length > 0 ? inchargeData.map((data, index) => 
                <Table.Tr key={index}>
                    ...
                    <Table.Td>
                        <Flex>
                            ...
                            <Flex gap="md">
                                <Anchor component={Link} href={`incharge/${data.id}/edit`} fz="md" fw={600} underline="never">Edit</Anchor>
                                <Anchor component={Link} href={`incharge/${data.id}/delete`} fz="md" fw={600} underline="never">Delete</Anchor>
                            </Flex>
                        </Flex>
                    </Table.Td>
                </Table.Tr>
