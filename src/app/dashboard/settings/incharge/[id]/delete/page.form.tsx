...
import { actionDeleteInchargeById } from "./page.form_action"
...
const inchargeDeleteFormSchema = yup.object().shape({
    user_password: yup.string().required("Password is required")
})

export default function InchargeDeleteForm({
    session_data,
    id
}: Readonly<{session_data: SessionData, id: number}>) {
    const [state, actionForm, pending] = useActionState(actionDeleteInchargeById, { message: ResponseDefaultMessage.None, response_data: "" })
    ...
    useEffect(() => {
        ...
        if (stateMessage == ResponseDefaultMessage.Success) {
            ...
            redirect("/dashboard/settings/incharge")
        }
    ...
    return <>
        <Formik
            ...
            validationSchema={inchargeDeleteFormSchema}
            onSubmit={(value) => {
                startTransition(() => {
                    actionForm({
                         data: {
        ...
        >
            {({values, errors, handleBlur, handleChange, touched, handleSubmit}) => (
                <Form>
                    ...
                    />
                    <Flex gap="md">
                        <Button component={Link} href={'/dashboard/settings/incharge'} variant="outline" size="xs" mt="sm">Cancel</Button>
