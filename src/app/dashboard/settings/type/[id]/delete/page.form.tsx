...
import { actionDeleteTypeById } from './page.form_action'
...
export default function TypeDeleteForm({
    session_data,
    id
}: Readonly<{ session_data: SessionData, id: number }>) {
    const [state, formAction, pending] = useActionState(actionDeleteTypeById, {message: ResponseDefaultMessage.None, response_data: ""})
    ...
    useEffect(() => {
        ...
        if (stateMessage == ResponseDefaultMessage.Success) {
            ...
            redirect("/dashboard/settings/type")
        }
        ...
        >
            {({values, errors, handleBlur, handleChange, touched, handleSubmit}) => (
                <Form>
                    ...
                    />
                    <Flex gap="md">
                        <Button component={Link} href={'/dashboard/settings/type'} variant="outline" size="xs" mt="sm">Cancel</Button>
