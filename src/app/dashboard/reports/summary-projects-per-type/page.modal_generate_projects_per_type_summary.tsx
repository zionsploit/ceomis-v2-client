...
import { generateProjectsPerTypeSummary } from "./page.modal_generate_projects_per_type_summary_action";
import { serverURL } from "@/provider/axiosClient";

export const OpenGenerateProjectsPerTypeSummary = (session_data: SessionData) => modals.open({
    title: <Text fz="lg" fw="bold" c="dimmed">GENERATE PROJECTS PER TYPE SUMMARY</Text>,
    size: rem("1000"),
    children: <GenerateProjectsPerTypeSummary session_data={session_data} />
})

const GenerateProjectsPerTypeSummary = ({session_data}: {session_data: SessionData}) => {
    const eventSourceRefId = useRef<EventSource | null>(null)
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const [_state, formAction, _pending] = useActionState(generateProjectsPerTypeSummary, "")
    ...
    return <>
        <Stack>
            <Stack gap={0}>
                <Text>Projects Per Type Summary</Text>
            </Stack>
            <Divider />
